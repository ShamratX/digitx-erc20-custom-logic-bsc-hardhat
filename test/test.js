...
describe("DigitX (custom logic only)", function () {
    ...
    describe("increaseAllowance (custom override)", function () {
        it("adds to current allowance", async function () {
            ...
            await token.connect(signerLp).approve(spender.address, amount);
            await token.connect(signerLp).increaseAllowance(spender.address, amount);
            expect(await token.allowance(ADDR_LP, spender.address)).to.equal(amount * 2n);
            await network.provider.request({
    ...
    describe("decreaseAllowance (custom override)", function () {
        it("subtracts from allowance", async function () {
            await network.provider.request({
                method: "hardhat_impersonateAccount",
                params: [ADDR_LP],
            });
            ...
            const [, , spender] = await ethers.getSigners();
            const signerLp = await ethers.getSigner(ADDR_LP);
            await token.connect(signerLp).approve(spender.address, ethers.parseEther("100"));
            await token.connect(signerLp).decreaseAllowance(spender.address, ethers.parseEther("30"));
            expect(await token.allowance(ADDR_LP, spender.address)).to.equal(ethers.parseEther("70"));
            await network.provider.request({
                method: "hardhat_stopImpersonatingAccount",
                params: [ADDR_LP],
            });
        });
        it("reverts when below zero", async function () {
            await network.provider.request({
                method: "hardhat_impersonateAccount",
                params: [ADDR_LP],
            });
            ...
            const [, , spender] = await ethers.getSigners();
            const signerLp = await ethers.getSigner(ADDR_LP);
            await token.connect(signerLp).approve(spender.address, ethers.parseEther("10"));
            await expect(
                token.connect(signerLp).decreaseAllowance(spender.address, ethers.parseEther("20"))
            ).to.be.revertedWith("BEP-20: decreased allowance below zero");
            await network.provider.request({
                method: "hardhat_stopImpersonatingAccount",
                params: [ADDR_LP],
            });
